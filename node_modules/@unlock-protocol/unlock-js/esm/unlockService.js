...
export default class UnlockService {
  ...
  providerForNetwork(networkId) {
    ...
    return new ethers.providers.JsonRpcProvider(this.networks[networkId].provider, networkId);
  }
  /**
  ...
  async _getPublicLockVersionFromContract(address, provider) {
    const contract = new ethers.Contract(address, ['function publicLockVersion() view returns (uint8)'], provider);
    let version = 0;
  ...
  async _getVersionFromContract(address, provider) {
    const contract = new ethers.Contract(address, ['function unlockVersion() view returns (uint8)'], provider);
    let version = 0;
