...
var _keyMetadata = require("./typedData/keyMetadata");

var _keyHolderMetadata = require("./typedData/keyHolderMetadata");
...
class WalletService extends _unlockService.default {
  ...
  async deployTemplate(version, callback) {
    const factory = new _ethers.ethers.ContractFactory(abis[version].PublicLock.abi, bytecode[version].PublicLock, this.signer);
  ...
  async deployUnlock(version, callback) {
    ...
    const factory = new _ethers.ethers.ContractFactory(abis[version].Unlock.abi, bytecode[version].Unlock, this.signer);
    const unlockContract = await factory.deploy({
      gasLimit: _constants.GAS_AMOUNTS.deployContract
    });
  ...
  async signMessage(data, method) {
    const dataHash = _utils.default.utf8ToHex(data);
  ...
  async setKeyMetadata({
    ...
    try {
      ...
      const payload = (0, _keyMetadata.generateKeyMetadataPayload)(currentAddress, metadata);
  ...
  async setUserMetadata({
    ...
    try {
      const currentAddress = await this.getAccount();
      const payload = (0, _keyHolderMetadata.generateKeyHolderMetadataPayload)(currentAddress, metadata);
      const signature = await this.unformattedSignTypedData(currentAddress, payload);
      const response = await fetch(url, {
        method: 'PUT',
        headers: {
  ...
  async getKeyMetadata({
    ...
    try {
      ...
      if (getProtectedData) {
        const currentAddress = await this.getAccount();
        const payload = (0, _keyMetadata.generateKeyMetadataPayload)(currentAddress, {});
        const signature = await this.unformattedSignTypedData(currentAddress, payload);
