...
var _v2 = _interopRequireDefault(require("./v6"));

var _v3 = _interopRequireDefault(require("./v7"));

var _v4 = _interopRequireDefault(require("./v8"));

var _v5 = _interopRequireDefault(require("./v9"));
...
class UnlockService {
  ...
  providerForNetwork(networkId) {
    ...
    return new _ethers.ethers.providers.JsonRpcProvider(this.networks[networkId].provider, networkId);
  }
  /**
  ...
  async _getPublicLockVersionFromContract(address, provider) {
    const contract = new _ethers.ethers.Contract(address, ['function publicLockVersion() view returns (uint8)'], provider);
    let version = 0;
  ...
  async _getVersionFromContract(address, provider) {
    const contract = new _ethers.ethers.Contract(address, ['function unlockVersion() view returns (uint8)'], provider);
    let version = 0;
    ...
    try {
      ...
      const hash = _ethers.ethers.utils.sha256(opCode);
  ...
  getContract(address, contract, provider) {
    return new _ethers.ethers.Contract(address, contract.abi, provider);
  }
