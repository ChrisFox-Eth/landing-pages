...
var _unlockService = _interopRequireDefault(require("./unlockService"));
 ...
 * }
 */
class Web3Service extends _unlockService.default {
  /**
  ...
  _create2Address(unlockAddress, templateAddress, account, lockSalt) {
    ...
    const byteCodeHash = _utils.default.sha3(byteCode);

    const seed = ['ff', unlockAddress, saltHex, byteCodeHash].map(x => x.replace(/0x/, '')).join('');

    const address = _utils.default.sha3(`0x${seed}`).slice(-40);

    return _utils.default.toChecksumAddress(`0x${address}`);
  }
  /**
  ...
  async generateLockAddress(owner, lock, network) {
    ...
    if (unlockContact.publicLockAddress) {
      ...
      const lockSalt = _utils.default.sha3(_utils.default.utf8ToHex(lock.name)).substring(2, 26); // 2+24


      return this._create2Address(this.networks[network].unlockAddress, templateAddress, owner, lockSalt);
    }

    return _ethers.ethers.constants.AddressZero;
  }
  /**
  ...
  async getAddressBalance(address, network) {
    ...
    return _utils.default.fromWei(balance, 'ether');
  }
  /**
  ...
  async recoverAccountFromSignedData(data, signedData) {
    return _utils.default.verifyMessage(data, signedData);
  }
  /**
  ...
  async getTokenSymbol(contractAddress, network) {
    const symbolPromise = (0, _erc.getErc20TokenSymbol)(contractAddress, this.providerForNetwork(network));
  ...
  async getTokenBalance(contractAddress, userWalletAddress, network) {
    const balance = await (0, _erc.getErc20BalanceForAddress)(contractAddress, userWalletAddress, this.providerForNetwork(network));
    const decimals = await (0, _erc.getErc20Decimals)(contractAddress, this.providerForNetwork(network));
    return _utils.default.fromDecimal(balance, decimals);
  }
  /**
