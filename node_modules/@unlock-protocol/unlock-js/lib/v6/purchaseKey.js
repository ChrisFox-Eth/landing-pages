...
async function _default({
  ...
  if (!keyPrice) {
    ...
    actualAmount = _utils.default.toDecimal(keyPrice, decimals);
  } else {
  ...
  if (erc20Address && erc20Address !== _constants.ZERO) {
    const approvedAmount = await (0, _erc.getAllowance)(erc20Address, lockAddress, this.provider, this.signer);

    if (!approvedAmount || approvedAmount.lt(actualAmount)) {
      await (0, _erc.approveTransfer)(erc20Address, lockAddress, actualAmount, this.provider, this.signer); // Since we sent the approval transaction, we cannot rely on Ethers to do an estimate, because the computation would fail (since the approval might not have been mined yet)
